...
#define TRACE     0    // PB0
#define PIN_PC7   23   // PC7
#define PIN_PC3   19   // PC3
...
void setup()
{
  ...
  pinMode(PIN_PC7, INPUT);  // spare input port
  digitalWrite(PIN_PC3, LOW);   // turn the output port low
  pinMode(PIN_PC3, OUTPUT); // spare output port
...
void loop()
{
  ...
  // Data out
  {
    ...
    for (uint16_t n = base_offset; n < (base_offset + 256); n++)
    {
      dataString += ",";
